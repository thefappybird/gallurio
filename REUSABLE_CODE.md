...
| `lib/billing/grantPlan.ts` | `grantPlan(workspaceId, { plan, expiresAt })` | Provider-agnostic plan grant: sets ONLY `Workspace.plan` + `planGrantExpiresAt`, never touches `ls*` fields. Use for any non-Lemon-Squeezy plan grant (beta tester, future promo codes) instead of a raw `Workspace.updateOne`. |
