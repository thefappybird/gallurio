...
| `lib/billing/availability.ts` | `isPaidBillingAvailable()` | Single source of truth for whether paid Lemon Squeezy checkout/portal are reachable, gated on `BETA_TESTER_ENABLED` (independent of `NODE_ENV`, matching `activateBetaTesterAction`). Call before any checkout/portal server logic and to gate the corresponding UI — used by `app/api/billing/checkout/route.ts`, `lib/actions/billing.ts`, `lib/env.ts`, and the onboarding plan / `/subscribe` / settings-billing pages. |
