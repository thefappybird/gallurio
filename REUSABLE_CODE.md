...
| `components/app/beta-plan-card.tsx` | `BetaPlanCard` | Presentational Beta plan card (label, name, "Free · while in beta" price, tagline, Pro's feature list, footnote). Never owns its CTA — caller renders its own activate button/state into `action`. Shared by onboarding plan step, settings billing panel, and subscribe (recovery) panel; also the target for the marketing-pages wave. | `action?: ReactNode`, `className?` |
