...
| `components/ui/image-cropper-dialog.tsx` | `ImageCropperDialog` | Modal image crop-before-upload dialog (`react-easy-crop` + canvas encode via `lib/media/cropImage.ts`). Locked to `CropSpec.aspect` (or the source image's own aspect when `aspect: null` — no free-rectangle mode exists in react-easy-crop); `cropShape="round"` when `spec.round`. Upload disabled until the first crop area arrives and while encoding. Don't instantiate directly — use `useImageCropper` below, which owns open/close + promise wiring. | `file: File \| null` (drives open), `spec: CropSpec`, `title?`, `description?`, `onCancel()`, `onConfirm(cropped: File)` |
...
| `lib/media/useImageCropper.tsx` | `useImageCropper` | `(spec: CropSpec, opts?: {title?, description?}) => { cropDialog: ReactNode, requestCrop(file: File) => Promise<CropRequestResult> }` | Owns crop-before-upload for any single upload surface: validates type/size against `spec` up front, bypasses the dialog entirely for `image/svg+xml` (vector, canvas can't rasterize), otherwise opens `ImageCropperDialog` and resolves once the user cancels or confirms. `CropRequestResult` is `{status:"ok",file}` \| `{status:"cancelled"}` \| `{status:"error",reason:"type_not_accepted"\|"file_too_large"}`. A second `requestCrop` call while one is pending resolves the prior one `cancelled` first (no leaked promise). Render `{cropDialog}` once anywhere in the tree. Crop specs live in `lib/media/cropSpecs.ts` (`CROP_SPECS.avatar/workspaceLogo/siteIcon/headerLogo/ogImage`); output encode is `lib/media/cropImage.ts`'s `cropToFile` (webp, downscaled to `spec.maxWidth/maxHeight`, never upscaled). |
