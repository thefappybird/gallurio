...
describe("POST /api/portfolio-maker-demo/upload", () => {
  it("returns 200 with imageId/uploadURL on a valid request", async () => {
    const res = await POST(makeReq({ demoSessionId: "session-1" }));
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json).toEqual({ imageId: "img_1", uploadURL: "https://upload.example/img_1" });
    expect(requestDirectUpload).toHaveBeenCalledWith("session-1", "portfolio-maker-demo");
  });
  ...
  it("returns image_cap_reached (400) on the 11th image for the same demoSessionId", async () => {
    for (let i = 0; i < 10; i += 1) {
      const res = await POST(makeReq({ demoSessionId: "session-cap" }, `10.0.0.${i + 1}`));
      expect(res.status).toBe(200);
    }
    requestDirectUpload.mockClear();
    const eleventh = await POST(makeReq({ demoSessionId: "session-cap" }, "10.0.0.99"));
  ...
  it("gives two different demoSessionIds independent 10-image budgets", async () => {
    for (let i = 0; i < 10; i += 1) {
      const res = await POST(makeReq({ demoSessionId: "session-a" }, `10.1.0.${i + 1}`));
      expect(res.status).toBe(200);
    }
    const capped = await POST(makeReq({ demoSessionId: "session-a" }, "10.1.0.99"));
    expect(capped.status).toBe(400);

    const otherSession = await POST(makeReq({ demoSessionId: "session-b" }, "10.1.0.100"));
    expect(otherSession.status).toBe(200);
  });

  it("returns 429 with Retry-After when the per-IP limit is exceeded", async () => {
    for (let i = 0; i < 30; i += 1) {
      const res = await POST(makeReq({ demoSessionId: `sess-${i}` }, "20.0.0.1"));
      expect(res.status).toBe(200);
    }
    const blocked = await POST(makeReq({ demoSessionId: "sess-overflow" }, "20.0.0.1"));
  ...
  it("returns 500 with a generic error body when requestDirectUpload throws", async () => {
    ...
    const res = await POST(makeReq({ demoSessionId: "session-err" }, "30.0.0.1"));
