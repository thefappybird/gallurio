...
type DraftShape = {
  version?: number;
  data?: Partial<Record<"home" | "gallery" | "navigation" | "footer", PuckData>>;
};

function readDraftZone(slug: string, zone: "home" | "gallery" | "navigation" | "footer", allowed: boolean): PuckData | null {
  if (!allowed) return null;
  try {
    const raw = window.localStorage.getItem(`gallurio:portfolio-draft:${slug}`);
    if (!raw) return null;
    const draft = JSON.parse(raw) as DraftShape;
    const zoneData = draft.version === LOCAL_DRAFT_VERSION || draft.version === 2 ? draft.data?.[zone] : undefined;
...
export function PreviewClient({
  ...
  slug: string;
  zone: "home" | "gallery" | "navigation" | "footer";
  ...
  const [data, setData] = useState<PuckData>(() => readDraftZone(slug, zone, allowBrowserRecovery) ?? fallbackData);
  ...
  useEffect(() => {
    ...
    setData(readDraftZone(slug, zone, allowBrowserRecovery) ?? fallbackData);
  }, [allowBrowserRecovery, fallbackData, slug, zone]);
