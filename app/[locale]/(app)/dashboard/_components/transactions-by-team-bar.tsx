...
import type { TransactionsByTeam } from "../_data/dashboard-metrics";

type Props = {
  data: TransactionsByTeam[];
  currency: string;
  locale: string;
  title: string;
  empty: string;
...
export function TransactionsByTeamBar({ data, currency, locale, title, empty }: Props) {
  const t = useTranslations("app.bookings.teamPicker");

  const rows = data.map((d) => ({
    name: d.isActive ? d.name : `${d.name} (${t("inactive")})`,
    total: d.total,
    color: d.color,
  }));
