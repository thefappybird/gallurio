...
- Transactions for any multi-document write that must succeed-or-fail together (inquiry → client → inquiry-status booking is the canonical example). MongoDB sessions, not application-level "rollback" logic.
...
- **Conversion model**: every inquiry submission creates an `Inquiry` + match-or-create `Client` + a `Booking` (`status: "inquiry"`, `createdFromInquiryId`) inside a Mongo transaction. The inquiry-status booking reads as a new-lead event; default bookings queries hide only `cancelled`. The owner approves in `/inquiries/[id]` → the **Create-Booking modal opens pre-filled** with the inquiry's client + event details, the owner adds pricing/deposit themselves, and on save the booking flips `status: "inquiry"` → `"booked"` and the inquiry is marked `converted`. There is **no in-app quoting, no `quoted` status, and no back-and-forth** — owner and client negotiate off-platform; Gallurio records only the final booking. See [`docs/booking-inquiry-lifecycle.md`](docs/booking-inquiry-lifecycle.md).
