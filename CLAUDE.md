...
- **Temporarily disabling the guard is allowed when serialization is the bottleneck** (user-authorized 2026-07-31): disable, fan out in parallel, then **re-enable in the same turn**. It is a globally-enabled plugin (`"tdd-guard@tdd-guard"` in `~/.claude/settings.json`), so this affects every project, not just the worktree — say so explicitly and treat re-enabling as an obligation. Known failure mode: on large rewrites the guard can contradict itself across successive edits; narrow the test to ONE assertion or escalate rather than burning cycles.
