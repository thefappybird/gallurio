...
- **i18n is in lockstep:** all five locales (`en/fil/ms/id/th`) removed the same two keys (`pipeline.quoted`, `statusValues.quoted`) and updated the CSV `status.note`. JSON remains valid (typecheck + tests load them). No locale left out of step.
