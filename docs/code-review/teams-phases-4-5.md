...
- **i18n**: all 25 new keys (teamPicker, calendar legend, wizard team label/placeholder, teams deactivate/reactivate dialogs, badges, errors, toasts, showDeactivated) verified present in **all four active** locales (en/fil/ms/id); stale `deleteDialog`/`team.delete`/`cannotDeleteDefault`/`toasts.deleted` keys removed from every locale.
