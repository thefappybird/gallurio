# Gallurio — Developer Reference

Detailed reference sections extracted from CLAUDE.md. Read the relevant section when working in that area. This file is not loaded by default — agents should read only the section they need.
...
- **Tenant isolation (RLS-equivalent)**: Mongo has no row-level security — your code is the only enforcement. See Multi-tenant rules in CLAUDE.md.
...
- Lemon Squeezy is the implemented subscription provider today. Checkout is synchronous: `POST /api/billing/checkout` authenticates, rate-limits, resolves the Pro variant, and returns a Lemon Squeezy-hosted checkout URL directly — no durable workflow/wait step. Lemon Squeezy, Creem, and a possible Paddle sole-proprietor application are under consideration for live launch; do not treat Creem or Paddle as integrated until an explicit selection and migration land. Pre-launch, `LEMONSQUEEZY_TEST_MODE` keeps the current implementation in test mode.
...
- `Workspace.plan` is `free|pro|beta`. Current Lemon Squeezy fields are `lsSubscriptionId`, `lsCustomerId`, `lsSubscriptionStatus`, `lsCurrentPeriodEnd`, and `lsLastEventAt`; a selected alternative MoR requires a reviewed schema and data migration, not a configuration toggle.
...
- Locales: `en`, `fil`, `id`, `ar` (Arabic is RTL), `th`. Malay (`ms`) dropped 2026-07-18; Thai (`th`) reintroduced 2026-07-18 after the original mojibake-corruption issue was root-caused and an encoding-sanity test was added.
