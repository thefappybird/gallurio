...
- [ ] Inventory production migrations/backfills. `scripts/backfill-inquiries.ts` now connects via `connectDB()` (`DATABASE_URL`) with a dev-target guard and `--dry-run` (hardened in the `action/beta-release-cleanup` branch); still confirm the target and dry-run before any production invocation.
