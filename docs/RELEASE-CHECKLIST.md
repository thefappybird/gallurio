...
- **Coming-soon gating while MoR verification is pending (2026-07-18):** the Pro card in the onboarding plan step, the public pricing page, and the landing pricing teaser show a "coming soon" overlay and cannot be selected/checked out. New users are routed to the existing beta-tester self-activation path (`activateBetaTesterAction`, `BETA_TESTER_ENABLED`) instead of Lemon Squeezy checkout for the duration of this window. Revert once a MoR clears live verification.
...
- [x] Confirm the launch locale decision: `en`, `fil`, `id`, `ar`, and `th`; Malay remains excluded.
...
- Launch locales are `en`, `fil`, `id`, `ar`, and `th`; Malay remains excluded. Arabic transactional email parity for lapse/lifecycle emails and RTL logo layout are implemented (2026-07-14); a full cross-surface locale sweep is still open.
