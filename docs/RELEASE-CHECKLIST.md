...
Last updated: 2026-07-13 (reconciled after PR #58 and Phase 1 decisions)
...
- The beta policy is a shared two-month beta run, not a separate timer per person. All active beta access ends together when the beta program is closed. A verified beta participant may redeem one two-month Pro promo at any time; the promo grant lasts two months from redemption. The repository currently has a perpetual `beta` promo path and does not yet record historical beta participation, so this policy is not code-complete.
...
- `subscription_payment_failed`, `past_due`, and `paused` are not by themselves sufficient to expire access. Gallurio expires access only when Lemon Squeezy reports its terminal non-payment state, or sends another provider-authoritative terminal event such as expiry/refund. The exact production payload/status mapping still requires verification against Lemon Squeezy.
...
- [ ] Implement and verify the beta-participant record, one-time promo redemption, two-month grant expiry, active-beta shutdown, stacking behavior with active Pro, and support/revocation handling.
...
- [ ] Verify the exact Lemon Squeezy terminal event/status payload in test mode and implement any missing mapping before production.
...
- [ ] Complete Arabic parity for transactional email copy and verify all five launch locales across production-facing surfaces.
...
- [ ] Pass `rtk vitest`, `pnpm test:integration`, `rtk tsc`, `rtk lint`, and `rtk next build` against the final release commit and retain the full CI logs. Verification on 2026-07-13 is not green: the unit suite timed out with an `EditorShell` failure, typecheck fails in `activity-feed.test.tsx`, lint has two errors, and integration/build cannot resolve the locally installed Workflow packages. Reinstall from the lockfile, fix genuine failures, and rerun before checking this item.
...
- [ ] Replace the perpetual beta promo path with the approved two-month Pro promo, persist an identity-level beta-participant record before beta closes, and enforce one redemption per eligible participant.
- [ ] Make the production beta-close operation global: stop new beta activation, end active beta access together, preserve all user/workspace data, and keep the participant promo redeemable according to the approved policy.
...
- The code now implements one free month, optional beta grants, active/canceled Pro, dunning retention, and a staged lapse lifecycle. The approved two-month global beta-close and participant-only two-month promo still require implementation and final legal/support approval.
...
- Launch locales are `en`, `fil`, `ms`, `id`, and `ar`; Thai remains excluded. Arabic transactional email parity is still open.
