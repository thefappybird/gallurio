...
- [x] Align the ordinary CI workflow with the Docker/release runtime: `.github/workflows/ci.yml` now uses Node 22, matching `Dockerfile` and `release.yml`. *(Done 2026-07-19; final CI evidence still belongs to the frozen release commit.)*
- [x] Fix the 3 current lint errors and diagnose the Vitest suite exceeding the prior 120-second review window. *(Done 2026-07-18: all 3 lint errors fixed without disabling rules — `booking-calendar.tsx`'s compact-view effect folded into its existing matchMedia subscribe-callback (no synchronous setState in a bare effect body); `booking-detail-modal.tsx`'s sessions `useMemo` now depends on a plain `bookingSessions` local instead of `booking?.sessions` optional-chaining; `SpotlightGuide.tsx`'s SSR-safe mount flag now uses `useSyncExternalStore` instead of an effect+setState. `pnpm lint`: 0 errors repo-wide (225 pre-existing warnings, unrelated). Root-caused and fixed one genuine flaky-test bug (`lib/storage/cloudflareImages.test.ts`: a rejection-matcher attached after the promise had already rejected, tripping a phantom `PromiseRejectionHandledWarning` on every run). The suite does NOT hang — a clean, uncontaminated full run completes in ~1903s (~32 min): 4778/4827 passing, 21/462 files with failures. The prior review's "120 seconds" was very likely a short tool/CI timeout rather than a suite expectation; ~32 minutes is a large-suite/serialized-MongoMemoryServer-per-file characteristic on this Windows dev box (`fileParallelism:false` is required here — see `vitest.config.ts`), not a bug to fix by itself. Spot-checked several of the 49 failing tests (`booking-detail-modal.tsx` Payments section, `messages/locale-parity.test.ts`) — confirmed unrelated to this session's changes (a `payments[].method` field drift, and a stale untranslated `ar.json` `betaEndingBanner` block respectively — the latter fixed in this pass). One pre-existing, unrelated bug found and left undisturbed: `onboarding/plan/plan-form.test.tsx` has 8 failing assertions caused by `PromoCodePanel` rendering twice (desktop+mobile breakpoint variants) with no CSS applied under this repo's `css:false` Vitest config, so role/text queries match two elements; confirmed via diff that no session code touches that path.

*(Formally confirmed test-by-test 2026-07-19: the full pass/fail list was diffed against `dev` (which sits exactly at this branch's 75-commit merge-base) file-by-file, not just spot-checked. Of the 49 originally-failing tests, 18 were genuine issues introduced by this branch's own 75 commits and are now fixed — 6 caused by one accidental-looking `lib/plans/entitlements.ts` edit (later found to be intentional, since its own dedicated `entitlements.test.ts` was updated in lockstep in the same commit; the 3 actually-stale downstream tests — `assertCanAddTeam.test.ts`, `dev.test.ts`, `app/[locale]/(app)/teams/_actions.test.ts` — were updated to match instead), and 12 were stale test assertions that hadn't been updated alongside legitimate feature work in this branch (the new `payments[].method` field, CID-embedded email logos replacing raw `<img src>` URLs, the forced-PNG Cloudflare email-logo variant, per-tag hash-based chip coloring, comma-or-whitespace SEO keyword tokenization, a lost viewport-height clamp on `UnsavedChangesDialog` from an `AlertDialog` refactor, the redesigned notification-burst-toast timing, and two copy/selector mismatches in brand-new team-sidebar and publish-dialog code) — see corresponding commits for the full list of touched files. The remaining 12 failures are confirmed pre-existing and unrelated to this branch: `lib/page-builder/seedPortfolio.test.ts` (1), `booking-detail-modal.test.tsx`'s "Session remove — scoped busy state" (1), `table-booking-manager.test.tsx` (2) — all three reproduce identically on `dev` in isolation — plus the 8 `plan-form.test.tsx` `PromoCodePanel` assertions documented above. Expected clean baseline: **4815/4827 passing, exactly these 12 pre-existing failures.** A final from-scratch full-suite run to physically re-confirm this exact number was attempted three times and killed partway through each time by what appears to be an environment/session constraint on this box, not a test failure — no failures were observed in any partial run before being killed. The 4815/4827 figure is derived from combining (a) the last fully-completed full run, which showed 14 failures matching this list before the entitlements correction was made, and (b) an isolated follow-up run of the 5 entitlements-affected files, which passed 43/43 after the correction — not from a single fresh green full run.)*
...
- The authoritative billing implementation is **Lemon Squeezy** today. Evidence: `@lemonsqueezy/lemonsqueezy.js`, `lib/lemonsqueezy/`, `app/api/billing/checkout/route.ts`, `app/api/webhooks/lemonsqueezy/route.ts`, and `docs/modules/billing.md`. Lemon Squeezy, Creem, and a possible Paddle sole-proprietor application are the three launch MoR candidates; only the provider selected after live eligibility and approval verification may be provisioned for production billing.
...
- [ ] Pass `rtk vitest`, `rtk tsc`, `rtk lint`, and `rtk next build` against the final release commit and retain the full CI logs. *(Progress 2026-07-19: typecheck clean (`rtk tsc`: no errors), lint clean (`rtk lint`: 0 errors, 225 pre-existing unrelated warnings). `pnpm test` baseline is now formally confirmed at 4815/4827 passing — see the test-by-test confirmation note under the short-list item above for the full methodology and the 12 remaining pre-existing failures (`seedPortfolio`, `booking-detail-modal.tsx` Session-remove, `table-booking-manager` ×2, `plan-form.tsx` PromoCodePanel ×8). `EditorShell.test.tsx`'s prior failure was a redundant, broken-by-construction duplicate of `BlockActionsToolbar.test.tsx`'s existing coverage (never mocked `getBoundingClientRect`, so it could never pass under happy-dom) and has been removed rather than fixed, since the behavior is already covered correctly elsewhere. `pnpm build` was not re-run in this pass. This has not yet been run against a frozen final release commit or retained as archived CI evidence — that step remains open.)*
...
- **Runtime installed:** Node `v22.22.1`, npm `9.2.0`, pnpm `11.5.2`, PM2 `7.0.3`, Git `2.53.0`, build tools, official-repository Caddy `2.11.4`, Docker Engine `29.6.2`, and Docker Compose `v5.3.1`. Docker is active and the restricted `gallurio` deployment user was verified able to run Docker/Compose. Ubuntu's packaged Corepack `0.24.0` crashed with Node 22, so Corepack is disabled and pnpm is installed directly at the recorded version.
- **Runtime transition:** Node/pnpm/PM2 are bootstrap-only; the final app runtime is Docker. `gallurio` has a passphrase-free GHCR `read:packages` credential stored at `~gallurio/.docker/config.json` (mode `0600`). Immutable image publication/deployment and Docker-managed boot recovery remain open.
- **Release layout:** `/opt/gallurio/compose.yml` is installed for the `gallurio` deployment user. `/etc/gallurio/gallurio.env` is root-owned, group-readable by `gallurio` (`0640`), and its read access was verified. It contains the prepared production runtime configuration, including beta-only mode. No production image or running Compose deployment exists yet.
...
- [x] Install Docker Engine and the Compose plugin, and configure the `gallurio` deployment user for the required Docker operations. *(Done 2026-07-19: Docker `29.6.2`, Compose `v5.3.1`, daemon active, and `gallurio` verified.)*
- [ ] Verify Docker-managed boot recovery after a first successful deployment.
- [x] Configure read-only GHCR authentication on the VPS. *(Done 2026-07-19: dedicated classic PAT with `read:packages` only; Docker config is `0600` and owned by `gallurio`.)*
- [ ] Publish the final Gallurio image from GitHub Actions using an immutable commit SHA/digest tag, then prove the VPS can pull it.
...
- [ ] Configure delivery/bounce/complaint monitoring and alerts. If Resend webhooks are added, verify signatures, make processing idempotent, and exclude the endpoint from Cloudflare challenges/caching. *(Progress: the production Resend endpoint is registered for `https://gallurio.com/api/webhooks/resend` and its signing secret is stored only in the VPS runtime environment. `app/api/webhooks/resend/route.ts` verifies the svix signature, dedupes via the shared `WebhookEvent` ledger, and logs `email.bounced`/`email.complained` events. The endpoint cannot be tested until the app is deployed; alert routing and Cloudflare challenge/cache exclusion remain open.)*
...
- [ ] Have two people compare production environment variable names—not values—against actual code reads and `.env.example`. *(Progress 2026-07-19: `/etc/gallurio/gallurio.env` was created with root ownership, `0640` permissions, and verified `gallurio` read access; runtime startup validation remains unproven until the first deployed image starts.)*
...
- **Priority:** blocking
- **Likely files/modules:** `lib/db/models/Workspace.ts`, `lib/plans/entitlements.ts`, `lib/page-builder/drafts.ts`, `lib/auth/assertCanAddTeam*`, `app/[locale]/(app)/settings/billing/_panel*`, teams plan props/components, portfolio draft actions, billing/webhook tests, `messages/{en,fil,id,ar,th}.json`, `docs/modules/billing.md`, and Lemon Squeezy docs. Do not confuse billing Starter with ordinary phrases such as “starter template” or “starter items.”
- **Current behavior after PR #58:** current billing types, entitlements, draft caps, checkout mappings, and launch UI use `free | beta | pro`; `2026-07-drop-starter-plan.ts` handles persisted Starter workspaces. Historical design documents still contain stale billing-tier references (the former `docs/dev-reference.md` did too — it was folded into `docs/modules/*.md` and no longer exists).
...
- The checked operational records above are the current evidence for production account/infrastructure setup. Public deployment, proxy-path verification, backup/alert evidence, and live-payment configuration remain explicitly open until tested.
