...
- **Language.** `proxy.ts` stamps `x-gallurio-portfolio-slug` (stripped of any inbound copy once at the top of `proxy()`), and the `(public)` root layout reads it to render real `lang` on `<html>`. `dir` on `<html>` is always `"ltr"` — RTL is scoped to the contact form and featured-work popup/Lightbox only, never to `<html>` or general block content (see "Navigation, footer, and chrome sync" below and `docs/modules/i18n-design.md`). Do **not** reintroduce a client-side `document.lang` patch — crawlers never see it.
...
- **Workspace logo**: no longer editable from `settings/public-page` — that upload control and its save path were removed when the header became a block. The portfolio nav logo uploader immediately inserts or updates an ordinary `Image` block inside the Navigation block's slot, so it lives in the published Puck data like any other block. New logo blocks use a 75x75px `contain` box by default; owners can keep it square or widen it into a rectangle with the Image block's width control. Navigation applies a 75px height cap to every nested Image so later manual resizing cannot grow the header. The 526x256 upload allowance does not determine the rendered shape. `publicPage.header` survives as **read-only legacy**: the load-time draft migration consumes it, public metadata reads its logo as a favicon fallback, and inquiry emails still read `header.logoUrl`. No code path writes it.
...
- **Manual nav reorder.** `PortfolioHeaderConfig.navOrder` is an optional permutation of `["logo","home","gallery","contact"]`, edited via `NavOrderControl`. `resolveNavOrder()` (`lib/page-builder/types.ts`) always returns a complete 4-key permutation — a missing/legacy/corrupt value degrades to the original order rather than dropping an item. The block never auto-mirrors its layout for RTL (`dir="ltr"` is hardcoded on the header); this manual control is the deliberate alternative for RTL-friendly layouts. See `REUSABLE_CODE.md`.

## Page structure (`PageBody`)

Ordinary content between Navigation and Footer always lives inside one locked, undeletable `PageBody` block — `normalizePageBody()` (`lib/page-builder/pageBody.ts`) enforces the canonical `[Navigation..., PageBody, Footer...]` shape on every load, folding loose blocks and duplicate bodies into the survivor. `PageBody` owns two things:
- `marginX` — the shared horizontal inset for all ordinary content. Effective-default pattern: stays unset in saved data, falling back to `PAGE_BODY_MARGIN_X_DEFAULT` (`1.5rem`) only at render (`lib/page-builder/blocks/PageBodyBlock.tsx`) — do not materialize it into `pageBodyDefaultProps`.
- `containerDefaults` — a future-only style preference (radius, padding, margin, gap, `overallWidth`) that `applyPageBodyContainerDefaults()` (`lib/page-builder/pageBodyContainerDefaults.ts`) materializes onto a Container **only at the moment it's newly inserted** inside this PageBody. Existing Containers are never touched, and the fields have no effect until a new one is dropped in.
...
Navigation is a twelfth drawer group with one neutral `NavigationPreset`. The retired Bordered, Underlined, and Scaled navigation component keys remain registered only as render compatibility for already-saved page data; they are not insertion choices.

The Navigation inspector intentionally has only **Content** and **Design** tabs. Content presents Brand, Banner, and Sync controls directly without nested drawers. Design contains the Links and Contact button drawers, with Links first and open by default. Navigation's generic `_style` Design and Layout controls are deprecated and hidden; existing `_style` data remains round-trip compatible but is ignored by `NavigationBlock`.

- **The two-level drawer is intentionally custom.** Puck 0.20 categories are flat (`category -> components`) and cannot express `Preset blocks -> group -> variants`. `EditorShell` therefore renders one Puck `Drawer` for drag-and-drop and nests ordinary collapsible group elements inside it. Preset rows open a localized live preview plus description on hover/focus; Manual block rows reuse the interaction but show localized descriptive text only.
...
- New Photo Grid and Masonry layouts own a `content` slot of ordinary `Image` blocks. Each photo therefore has its own picker, shared photo-details editor, crop/size controls, and Puck drag order. Description supplies both visible copy and alt text. Legacy `images[]` gallery data remains read-only compatibility input for previously saved pages; do not seed it in new presets. `FeaturedWork` is retained only for saved pages, while every new collection composition uses `Columns` plus `CollectionCard` blocks.
...
`blocks/presetContrast.test.ts` measures all 33 Container presets against all 6 committed brand kits (WCAG 2.1), modelling `resolveBlockStyle` and every `ButtonBlock` branch, plus the scrim case that only appears once an owner adds a background image. It is coupled to those renderers on purpose: if either changes, this test changes with it.
...
## Featured Work popup & image modal layouts

The Featured Work collection popup and every standalone image modal (photo grids, masonry, Image blocks) are each independently selectable from four layouts — Contact Sheet, Justified, Split Index, Immersive — via `popupLayout`/`imageModalLayout` on `PortfolioCollectionsPopupConfig` (`e3c4b6d8`+). `imageModalLayout` governs every image modal on the page (not just the featured popup) and stays active even under an `immersive` popup layout, since Immersive replaces only the popup's own modal. Both are chosen through the shared `LayoutPicker` (see `REUSABLE_CODE.md`), which previews real workspace photos rather than placeholders.

- **Popup columns.** `PortfolioCollectionsPopupConfig.popupColumns` (optional integer 1-6, `resolvePopupColumns()` resolves unset/legacy values to `3`) sets the image column count for Contact Sheet, Justified, and Split Index. Immersive neither reads nor exposes the control, but the value is preserved in config while Immersive is selected. See `REUSABLE_CODE.md`'s `lib/page-builder/types.ts` entry.
- **Shared chrome.** `DotPagination` (`lib/page-builder/blocks/imageModal/DotPagination.tsx`) replaces a numeric counter with clickable position dots for collections of 8 or fewer photos, shared across Caption, Cinema, and Immersive. `SeeMoreMetaPanel` (`lib/page-builder/blocks/imageModal/SeeMoreMetaPanel.tsx`) is the collapsed-by-default disclosure for date/location/client/custom metadata/tags on the scrim-backed Caption and Cinema layouts. `ImmersiveViewer` (`lib/page-builder/blocks/ImmersiveViewer.tsx`) is the shared full-screen viewer behind both the Immersive popup and the Cinema image-modal layout.
- **RTL.** All four popup layouts and the nested `Lightbox` follow the contact-form/popup RTL scoping described above (owner's `formLocale`/`formDir`, never the CRM UI locale or `<html>`).
...
- `GalleryItem`: individual photo/media asset — Cloudflare `assetId`/`assetProvider`, delivery `url`, dimensions, description (`caption`, also used as alt text), legacy `altText`, `order`, `tags[]`, link metadata, and the `hideClient` privacy flag.
