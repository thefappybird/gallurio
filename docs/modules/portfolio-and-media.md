...
Exactly 3 public pages per workspace: Home, Gallery, Contact, served at `/w/[orgSlug]`. Source of truth is `Workspace.publicPage` (template id, `data.{home,gallery,navigation,footer}`, `brandKit`, `savedThemes[]`, SEO fields, `formLocale`/`formDir`, per-section style overrides). Navigation and footer are shared Puck documents rendered around Home and Gallery in the editor, preview, and public layout; they are stored with each durable draft and promoted only with the selected draft on Publish. A shared Puck config powers both the in-app editor and the public renderer — one block definition, two consumers. Do not add portfolio collections beyond Home/Gallery/Contact without explicit approval.
...
- Footer variants remain ordinary Puck compositions, but new and migrated footer content belongs to the draft's shared `data.footer` document. Page-local footer presets are stripped from Home/Gallery rendering to prevent duplicate chrome. The legacy page-local shape remains migration input only.
...
- Photo Grid and Masonry do not expose container background-image, background-animation, scrim, or background-opacity controls. Previously stored backgrounds still render for visual compatibility. Their multi-photo picker is replacement-oriented: its fixed limit is the number of current Image slots in that block, and the selected-count status stays visible while browsing, uploading, selecting, or reordering.
...
- `PortfolioDraft`: named, durable draft snapshots (`{ home, gallery, navigation, footer }` Puck data + brandKit/contact/header/collectionsPopup/SEO), unique per `(workspaceId, name)`. The editor opens the newest saved draft by `updatedAt`; if none exists it falls back to published content, then scratch. Browser localStorage is only a temporary recovery buffer: valid recovery is offered explicitly, applied only after Resume, cleared after a successful save/publish, and not recreated until a later edit. Preview resolves the selected durable draft and reads that recovery buffer only when the editor explicitly opts in.
