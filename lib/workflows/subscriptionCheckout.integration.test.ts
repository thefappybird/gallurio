...
beforeAll(async () => {
  ...
  // Paddle price env vars — no longer used by the step but kept in case
  // the test environment is shared with other suites.
...
describe("subscriptionCheckoutWorkflow", () => {
  it("persists subscription bookkeeping fields when the hook is resumed", async () => {
    ...
    const ws = await Workspace.create(
      makeWorkspaceBase({
        paddleCheckoutWorkflowRunId: "wrun_test_initial",
      })
    );
    ...
    // Wait for the hook to be created, then resume it with a Paddle event.
    await waitForHook(run, { token: `paddle-checkout-${wsId}` });

    const periodEnd = new Date(Date.now() + 30 * 86_400_000).toISOString();
    await resumeHook(`paddle-checkout-${wsId}`, {
    ...
    // fields and clear paddleCheckoutWorkflowRunId. `plan` is set by the
    ...
    expect(updated!.paddleCurrentPeriodEnd).toBeInstanceOf(Date);
    // clearCheckoutRunStep must have unset this field
    expect(updated!.paddleCheckoutWorkflowRunId).toBeFalsy();
  });
  ...
  it("does not touch a second workspace — tenant isolation", async () => {
    ...
    const wsA = await Workspace.create(
      makeWorkspaceBase({
        paddleCheckoutWorkflowRunId: "wrun_ws_a",
        plan: "free",
      })
    );
    ...
    await waitForHook(run, { token: `paddle-checkout-${wsAId}` });

    const periodEnd = new Date(Date.now() + 30 * 86_400_000).toISOString();
    await resumeHook(`paddle-checkout-${wsAId}`, {
    ...
    expect(updatedA!.paddleSubscriptionId).toBe("sub_2");
    expect(updatedA!.paddleSubscriptionStatus).toBe("active");

    // wsB must be entirely unmodified
    const updatedB = await Workspace.findById(wsBId).lean();
    expect(updatedB!.plan).toBe("free");
    expect(updatedB!.paddleSubscriptionId).toBeNull();
    expect(updatedB!.paddleCustomerId).toBeNull();
    expect(updatedB!.paddleSubscriptionStatus).toBeNull();
  });

  it("normalises an unknown paddle status to null", async () => {
  ...
  it("still writes subscription fields even when status is unknown", async () => {
    ...
    expect(updated!.paddleSubscriptionId).toBe("sub_4");
    expect(updated!.paddleCustomerId).toBe("ctm_4");
  });
});
