 ...
 *
 * Two heights, matching the only two cases in which the anchor exists in the
 * data at all (see shouldKeepAnchor — a container holding ANY ordinary child
 * carries no anchor):
 *  - empty container: the full editor footprint, so the container is droppable.
 *  - all children container-class: a 4px bridge, so a sibling can land beside
 *    the nested container/columns instead of inside it.
 *
 * A3 note: NO ResizeObserver and no DOM-size measurements. Height comes purely
 * from Puck store data (props.minHeight + content length), so there is no
 * feedback loop here that could contribute to the Columns oscillation crash
 * described in items 3/4/6. The root cause of that crash — shared containerName
 * "pf-cols" causing cross-instance @container rule contamination — was fixed in
 * ColumnsBlock (A1).
 */
export function EditorContainerAnchor({ id }: { id: string }) {
  const parentId = id.replace(/--anchor$/, "");

  // Reactively compute the anchor's height from the parent's live content
  ...
  const height = usePuckStore((s): number | null => {
    ...
    if (realChildren.length === 0) return CONTAINER_EDITOR_HEIGHT_PX[minHeight];
    // Reachable only for an all-container-class slot; a stale anchor left in a
    // mixed slot by an older draft renders as the same thin bridge until the
    // reconciler strips it on the next store tick.
    return realChildren.every((child) => isContainerClass(child.type)) ? 4 : null;
  });
